...
function Navbar() {
  ...
  return (
    <div className='rounded-div flex items-center justify-between h-20 font-bold'>
        <Link to='/'>
            <h1 className='text-2xl'>CryptoCoins Base</h1>
        </Link>
        ...
        {user?.email ? (
        <div>
          <Link to='/account' className='border px-6 py-2 rounded-2xl shadow-lg hover:shadow-2xl'>
            Account
          </Link>
          <button className='bg-button text-btnText px-5 py-2 ml-2 rounded-2xl shadow-lg hover:shadow-2xl' onClick={handleSignOut}>Sign out</button>
        </div>
      ) : (
        <div className='hidden md:block'>
